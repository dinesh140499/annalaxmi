...
const categorySalesDistribution = [
  { category: 'Organic Pulses & Dals', percent: 38, sales: '₹42,800', color: 'bg-emerald-500' },
  { category: 'Cold-Pressed Virgin Oils', percent: 26, sales: '₹29,400', color: 'bg-amber-500' },
  { category: 'Ancient Grains & Rice', percent: 20, sales: '₹22,600', color: 'bg-blue-500' },
...
const AdminDashboard = () => {
  ...
  const quickStats = [
    {
      ...
      icon: <FaReceipt className="text-xl text-emerald-400" />,
      color: 'from-emerald-950/60 to-slate-900 border-emerald-800/40',
      link: '/admin/orders',
    },
    {
      ...
      icon: <FaBoxes className="text-xl text-amber-400" />,
      color: 'from-amber-950/40 to-slate-900 border-amber-800/40',
      link: '/admin/products',
    },
    {
      ...
      icon: <FaUsersCog className="text-xl text-emerald-400" />,
      color: 'from-emerald-950/60 to-slate-900 border-emerald-800/40',
      link: '/admin/admins',
    },
    {
      ...
      icon: <FaTags className="text-xl text-blue-400" />,
      color: 'from-blue-950/40 to-slate-900 border-blue-800/40',
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto font-sans">
      ...
      <div className="bg-gradient-to-r from-emerald-950 via-slate-900 to-slate-950 border border-emerald-800/40 rounded-3xl p-6 sm:p-8 shadow-xl relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-1.5 text-amber-400 text-xs font-bold uppercase tracking-wider bg-amber-500/10 border border-amber-500/20 px-3 py-1 rounded-full mb-3">
          ...
          <div className="flex items-center gap-3">
            {isSuperAdmin && (
              <Link
                to="/admin/admins"
                className="bg-emerald-600 hover:bg-emerald-500 active:scale-95 text-white font-bold py-2.5 px-5 rounded-xl text-xs sm:text-sm flex items-center gap-2 shadow-lg transition cursor-pointer"
              >
                <FaUsersCog />
                <span>Manage Staff & RBAC</span>
              </Link>
            )}
            <Link
              to="/admin/orders"
              className="bg-slate-800 hover:bg-slate-700 text-white font-bold py-2.5 px-4 rounded-xl text-xs sm:text-sm flex items-center gap-2 border border-slate-700 transition"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        {quickStats.map((stat, i) => (
          <Link
            key={i}
            to={stat.link}
            className={`bg-gradient-to-b ${stat.color} border rounded-2xl p-5 shadow-xs hover:shadow-lg hover:border-emerald-500/60 transition group flex flex-col justify-between`}
          >
            <div>
              ...
              <div className="text-2xl sm:text-3xl font-black text-white group-hover:text-emerald-300 transition">
                {stat.value}
              </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-800 flex items-center justify-between text-xs font-semibold text-slate-400 group-hover:text-white">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xs space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-white">Live Orders Fulfillment Feed</h3>
              </div>
              <Link to="/admin/orders" className="text-xs font-bold text-emerald-400 hover:underline">
                ...
                <div key={ord.id} className="py-3 flex items-center justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-slate-300">{ord.customer}</span>
                    </div>
                    <p className="text-[11px] text-slate-400 mt-0.5">{ord.items}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-black text-white block">₹{ord.amount}</span>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xs space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {isSuperAdmin && (
                <Link
                  to="/admin/admins"
                  className="bg-slate-950 hover:bg-slate-850 border border-slate-800 hover:border-emerald-500/50 rounded-2xl p-4 transition group flex items-start gap-3"
                >
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="bg-slate-950 rounded-xl p-3 border border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="font-semibold text-slate-300">Rate Limiter Protection</span>
                </div>
                <span className="inline-flex items-center gap-1 text-emerald-400 font-bold">
                  <FaCheckCircle className="text-[10px]" />
